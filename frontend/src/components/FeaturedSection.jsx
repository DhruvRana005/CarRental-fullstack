...
const FeaturedSection = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: "easeOut" }} className='flex flex-col items-center py-24 px-6 md:px-16 lg:px-24 xl:px-32'>
      ...
      <motion.div
        initial={{ opacity: 0, y: 100 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1, duration: 1 }} className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-18'>
        {cars.length > 0 ? (
          cars.slice(0, 6).map((car) =>
            <motion.div key={car._id}
              initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: "easeOut" }}>
              <CarCard car={car} />
            </motion.div>
          )
        ) : (
          <p className="text-center col-span-full text-gray-500">No featured cars available.</p>
        )}
      </motion.div>
      <motion.button initial= {{ opacity: 0, y: 20}} whileInView={{ opacity: 1, y: 0}} transition={{ delay: 0.6, duration: 0.4 }}
